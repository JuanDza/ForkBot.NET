    ...
    public abstract class TradeCordBase<T> where T : PKM, new()
    {
        ...
        protected bool CreateDB()
        {
            ...
            if (!exists)
            {
                ...
                for (int i = 1; i < (Game == GameVersion.BDSP ? 494 : 900); i++)
                {
                    cmd.CommandText = "insert into dex_flavor(species) values(@species)";
                    cmd.Parameters.AddWithValue("@species", i);
                    cmd.ExecuteNonQuery();

                    TradeExtensions<PK8>.FormOutput(i, 0, out string[] forms);
                    for (int f = 0; f < forms.Length; f++)
                    {
                        var name = SpeciesName.GetSpeciesNameGeneration(i, 2, 8);
                        bool gmax = Game == GameVersion.SWSH && new ShowdownSet($"{name}{forms[f]}").CanToggleGigantamax(i, f);
                        string gmaxFlavor = gmax ? DexText(i, f, true) : "";
                        string flavor = "";
                        try
                        {
                            flavor = DexText(i, f, false);
                        }
                        catch
                        {
                            Base.LogUtil.LogText("oops");
                        }
            ...
            catch
            {
                ...
                foreach (var entry in catches)
                {
                    ...
                    cmd.Parameters.AddWithValue("@is_gmax", set.CanGigantamax ? 1 : 0);
        ...
        protected bool BaseCanBeEgg(int species, int form, out int baseSpecies)
        {
            baseSpecies = -1;
                            ...
                            case CheckIdentifier.Evolution:
                                {
                                    if (pk.Species == (int)Lickilicky && pk.Met_Location != 162 && pk.Met_Location != 244 && !pk.RelearnMoves.Contains(205))
                                        SetMoveOrRelearnByIndex((T)pk, 205, true);
                                }; break;
                            ...
                            case CheckIdentifier.Form:
                                {
                                    if (pk.Species == (int)Keldeo && pk.Form == 1 && !pk.Moves.Contains(548))
                                        SetMoveOrRelearnByIndex((T)pk, 548, false);
                                }; break;
