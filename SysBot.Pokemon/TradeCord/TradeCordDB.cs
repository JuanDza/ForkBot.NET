...
namespace SysBot.Pokemon
{
    public abstract class TradeCordDatabase<T> : TradeCordBase<T> where T : PKM, new()
    {
        ...
        protected T EggRngRoutine(IReadOnlyList<EvoCriteria> evos, int[] balls, int generation, string trainerInfo, Shiny shiny)
        {
            ...
            {
                if (BaseCanBeEgg(evos[i].Species, evos[i].Form, out baseSpecies) && baseSpecies > 0)
                {
                    formID = evos[i].Form;
                    break;
                }
            ...
            if (random)
            {
                while (true)
                {
                    ...
                    if (BaseCanBeEgg(speciesID, formID, out baseSpecies) && baseSpecies > 0)
                    {
                        formName = formsR[formID];
            ...
            var set = new ShowdownSet($"Egg({speciesName}{formName})\nBall: Poke{shinyRng}\n{trainerInfo}");
        ...
        protected void EventHandler(TradeCordSettings settings, out MysteryGift? mg, out int form)
        {
            ...
            do
            {
                ...
                {
                    BaseCanBeEgg(Rng.SpeciesRNG, 0, out int baseSpecies);
